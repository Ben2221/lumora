...
export default function Top10MediaCard({ item, index }: { item: MediaItem; index: number }) {
  ...
  return (
    <div 
      className="relative group shrink-0 flex items-center justify-end w-[200px] sm:w-[240px] md:w-[280px] aspect-[2/3] transition-transform duration-300 hover:scale-105 hover:z-30 cursor-pointer"
      onMouseEnter={() => setIsHovered(true)}
      onMouseLeave={() => setIsHovered(false)}
    >
      <Link href={`/watch/${item.type}/${item.id}`} className="relative block w-[70%] h-full rounded-md overflow-hidden bg-gray-900 shadow-xl z-20">
        <Image
        ...
        <div className={`absolute inset-0 bg-black/60 transition-opacity duration-300 flex flex-col justify-end p-4 ${isHovered ? 'opacity-100' : 'opacity-0'}`}>
          ...
          <div className="relative z-10 translate-y-4 group-hover:translate-y-0 transition-transform duration-300">
            ...
            <div className="flex items-center gap-2 mt-1">
              <span className="text-green-500 font-medium text-[10px]">{item.vote_average * 10}%</span>
            </div>
          </div>
        </div>
      </Link>
      ...
      <div 
        className="absolute left-[-5%] sm:left-[-10%] bottom-[-10%] text-black font-black tracking-tighter leading-none z-10 pointer-events-none select-none"
        style={{
